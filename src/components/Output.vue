<template>
    <FlexibleRow>
        <template #title>
            ...
            <RowDescription
                v-if="row.description[language]"
                :description="row.description[language]">
            </RowDescription>
            <RowDescription
                v-if="row.warning[language]"
                :description="row.warning[language]"
                type="warning">
            </RowDescription>
        </template>
        <template #years>
    ...
    export default {
        ...
        methods: {
            formatValueForFiscalYear(valueForFiscalYear, fiscalYear) {
                ...
                const column = Object.keys(this.workbookStore.fiscalYears).find(
                    (col) => this.workbookStore.fiscalYears[col] === fiscalYear,
                );
                ...
                const outputValue =
                    this.workbookStore?.processed?.outputs?.[cell] ?? 0;

                return (
                    this.row.unit === "percents"
                        ? outputValue * 100
                        : outputValue
                ).toFixed(this.roundTo);
            },
        },
    };
</script>
