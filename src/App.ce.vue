<template>
    <div
        ...
        <nav class="flex flex-row items-center justify-end gap-4 print:hidden">
            <Button
                type="negative"
                @click="clear"
                :disabled="!workbookStore.isDirty"
                >{{ strings.clearUserInput }}</Button
            >
        </nav>

        <LoadingIndicator
            class="h-8 w-8"
            v-if="workbookStore.loading"></LoadingIndicator>
        <ErrorBlock v-if="workbookStore.error"></ErrorBlock>
        <ErrorBlock v-if="workbookStore.isBeingMaintained"></ErrorBlock>
        <main
            v-if="
                !workbookStore.loading &&
                !workbookStore.error &&
                !workbookStore.isBeingMaintained
            ">
            <FlexibleRow
                class="sticky top-0 hidden border-b border-solid border-gray-300 bg-white md:grid dark:bg-gray-900"
                aria-hidden="true">
                <template #title> </template>
                <template #years>
                    <div
                        v-for="(year, column) in workbookStore.fiscalYears"
                        v-text="year"
                        class="text-right font-semibold print:text-xs"></div>
                </template>
            </FlexibleRow>
        ...
        -->
            <section
                class="flex flex-col divide-y divide-solid divide-blue-100 dark:divide-blue-800">
                <FlexibleRow
                    :editable="true"
                    v-for="input in workbookStore.inputs">
                    <template #title>
                        {{ input.label[language] }}
                        <Unit v-if="input.unit">{{
                            strings[`units_${input.unit}`]
                        }}</Unit>
                    </template>
                    <template #years>
                        <div
                            v-for="(userValue, fiscalYear) in workbookStore
                                .userValues[input.id]">
                            <Field
                                v-model="userValue.value"
                                :label="fiscalYear"
                                @input="handleUpdatedUserInput"
